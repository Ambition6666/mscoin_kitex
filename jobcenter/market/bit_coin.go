...
func (b *BitCoin) ScanTx() {
	...
	dealBlocksStr := b.ch.Get(context.Background(), "BTC::TX").String()
	...
	address, err := b.assetRpc.GetAddress(ctx, &ucenter.AssetReq{
		CoinName: "BTC",
	})
	if err != nil {
		log.Println(err)
	...
	//11. 记录redis的区块高度
	b.ch.Set(context.Background(), "BTC::TX", currentBlocks, -1)
	b.wg.Done()
}
...
func NewBitCoin() *BitCoin {
	return &BitCoin{
		ch:            utils.GetRedis(),
		assetRpc:      rpc.GetAssetClient(),
		bitCoinDomain: domain.NewBitCoinDomain(utils.GetMongoClient()),
		queueDomain:   domain.NewQueueDomain(utils.GetRocketMQProducer()),
	}
}
