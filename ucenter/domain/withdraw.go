...
type WithdrawDomain struct {
	withdrawRecordRepo *dao.WithdrawDao
	memberWalletDomain *MemberWalletDomain
	marketRPC          market.Client
}

func NewWithdrawDomain() *WithdrawDomain {
	return &WithdrawDomain{
		withdrawRecordRepo: dao.NewWithdrawDao(),
		memberWalletDomain: NewMemberWalletDomain(),
		marketRPC:          rpc.GetMarketClient(),
	}
}
func (d *WithdrawDomain) SaveRecord(ctx context.Context, conn *gorm.DB, wr *model.WithdrawRecord) error {
